...
/**
 * @DI\Service("sofachamps.listener.referral_listener")
 */
class ReferralListener
{
    const REFERRAL_COOKIE  = 'sofachamps_referrer';
    const REFERRAL_PARAM   = 'rid';
    const REFERRAL_SESSION = 'sofachamps.referrer';
     ...
     *      "referralManager" = @DI\Inject("sofachamps.referrer.referral_manager")
     * })
     */
    ...
    public function onKernelRequest(GetResponseEvent $event)
    {
        ...
        if ($referralId = $request->query->get(self::REFERRAL_PARAM)) {
    ...
    public function addReferrerToUser(FilterUserResponseEvent $event)
    {
        ...
        // Find the referrer
        $referrer = $this->em->getRepository('SofaChampsCoreBundle:User')->find($referralId);
